...
/*
TODO: Some variables like non_delta_trigger will need to be arrays for each slot. Decide which.

my_GhoulPackReliable() - saves the ghoulChunks.

my_SV_ExecuteClientMessage() - if (non_delta_trigger) sets cl->lastframe = -1

my_SV_SendClientDatagram() - disables non_delta_trigger because frame is sent.
...
my_Netchan_Transmit_Save - demo_system.demo_recorder->record(chan,relLen,relBuf,length,data);

cmd_nix_stoprecord - demo_system.demo_recorder.finish();

cmd_nix_record - demo_system.demo_recorder.begin();
...
void DemoRecorder::record(void * netchan, int relLen, unsigned char * relData, int unrelLen, unsigned char * unrelData)
{
...
/*
	sizebuf_t buf is just a temporary location.
	it gets written into chunk_t.
	should return a vector of chunk_t
...
*/
void DemoRecorder::generate_svc_serverdata(std::vector<chunk_t>& resultVector) {
