...
{
	#if 0
	std::string output_file_path = out_root + std::string("/");
	// create_file_dir_if_not_exists(output_file_path.c_str());
	int ret = minizip_extract(in_zip, NULL, output_file_path.c_str(), NULL, NULL);
	if (ret == MZ_OK) {
		SOFPPNIX_DEBUG("Success extracting!!!\n");
		return true;
	}
	else {
		SOFPPNIX_DEBUG("Error extracting!!!\n");
		return false;
	}
	#endif
